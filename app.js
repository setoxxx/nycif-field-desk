...
const STORAGE_KEY = 'nycif-field-desk-state-v03';
...
function photoPriority(event) {
  if (isNypd(event) || event.crowd_level === 'very_high' || event.priority >= 1100) return 'Must cover';
...
function assignmentText(event) {
  const lines = ['NYCIF FIELD ASSIGNMENT', event.title, `Time: ${timeLabel(event.start)}`, event.borough ? `Borough: ${event.borough}` : '', event.location ? `Location: ${event.location}` : '', distanceLabel(event) ? `Distance: ${distanceLabel(event)}` : '', `Priority: ${photoPriority(event)}`, crowdLabel(event) ? `Assignment read: ${crowdLabel(event)}` : '', isNypd(event) ? 'NYPD intel item' : '', event.nypd_notice ? `NYPD note: ${event.nypd_notice}` : '', `Apple Maps: ${appleMapsUrl(event)}`, `Google Maps: ${googleMapsUrl(event)}`, (event.source_url || event.url || event.event_url) ? `Source: ${event.source_url || event.url || event.event_url}` : ''];
...
async function loadFeed(kind) { const url = FEEDS[kind]; status(`Loading ${kind === 'major' ? 'major field assignments' : 'full event database'}…`); const response = await fetch(`${url}?cache=${Date.now()}`, { headers: { Accept: 'application/json' } }); if (!response.ok) throw new Error(`${kind} feed HTTP ${response.status}`); const json = await response.json(); const rows = Array.isArray(json) ? json : (json.events || []); const events = rows.map(makeEvent).filter(Boolean); if (kind === 'major') { state.events = events; state.feed = 'major'; } else { const existing = new Map(state.events.map(e => [e.id, e])); events.forEach(event => existing.set(event.id, event)); state.events = [...existing.values()]; state.feed = 'full'; state.fullLoaded = true; } const visible = render(); if (visible.length) map.fitBounds(visible.slice(0, 200).map(e => [e.lat, e.lng]), { padding: [44, 44], maxZoom: 12 }); setTimeout(() => map.invalidateSize(), 120); }
function syncUiFromState() { els.majorOnly.checked = state.majorOnly; els.photoOnly.checked = state.photoOnly; els.nypdOnly.checked = state.nypdOnly; els.sortSelect.value = state.sort; document.querySelectorAll('[data-cat]').forEach(input => { input.checked = !!state.categories[input.dataset.cat]; }); }
...
function buildBoroughs() { els.boroughs.innerHTML = BOROUGHS.map(b => { const value = b === 'All' ? 'all' : b; return `<button type="button" class="${state.borough === value ? 'active' : ''}" data-borough="${esc(value)}">${esc(b)}</button>`; }).join(''); els.boroughs.addEventListener('click', ev => { const button = ev.target.closest('[data-borough]'); if (!button) return; state.borough = button.dataset.borough; els.boroughs.querySelectorAll('button').forEach(b => b.classList.toggle('active', b === button)); savePrefs(); render(); }); }
function buildDateChips() { if (!els.dateChips) return; els.dateChips.querySelectorAll('[data-date-mode]').forEach(button => button.classList.toggle('active', button.dataset.dateMode === state.dateMode)); els.dateChips.addEventListener('click', ev => { const button = ev.target.closest('[data-date-mode]'); if (!button) return; state.dateMode = button.dataset.dateMode; els.dateChips.querySelectorAll('[data-date-mode]').forEach(b => b.classList.toggle('active', b === button)); savePrefs(); render(); }); }
function setUserLocation(latitude, longitude, accuracy) { const here = [latitude, longitude]; state.userLocation = { lat: latitude, lng: longitude }; if (userMarker) userMarker.setLatLng(here); else { const icon = L.divIcon({ className: 'user-location-shell', html: '<span class="user-location">🗽</span>', iconSize: [36, 44], iconAnchor: [18, 42], popupAnchor: [0, -38] }); userMarker = L.marker(here, { icon, zIndexOffset: 4000 }).addTo(map).bindPopup(`<strong>You are here</strong><br>Accuracy: ${Math.round(accuracy || 0)} meters`); } if (userAccuracy) { userAccuracy.setLatLng(here); userAccuracy.setRadius(accuracy || 0); } else { userAccuracy = L.circle(here, { radius: accuracy || 0, color: '#d40000', weight: 2, fillColor: '#d40000', fillOpacity: 0.08 }).addTo(map); } }
function locateUser(options = {}) { if (!navigator.geolocation) { status('Location is not available in this browser.'); return; } status('Finding your location…'); navigator.geolocation.getCurrentPosition(pos => { const { latitude, longitude, accuracy } = pos.coords; setUserLocation(latitude, longitude, accuracy); if (options.sortNear) { state.sort = 'near'; els.sortSelect.value = 'near'; savePrefs(); } map.flyTo([latitude, longitude], Math.max(map.getZoom(), 14), { duration: 0.6 }); userMarker.openPopup(); render(); status(options.sortNear ? 'Sorted assignments near you.' : 'Location updated.'); }, err => status(`Location failed: ${err.message}`), { enableHighAccuracy: true, timeout: 12000, maximumAge: 15000 }); }
function bindUi() { els.layersBtn.addEventListener('click', () => setLayers(els.layersPanel.hidden)); els.deskBtn.addEventListener('click', () => setDesk(els.deskDrawer.hidden)); els.closeDeskBtn.addEventListener('click', () => setDesk(false)); els.locateBtn.addEventListener('click', () => locateUser()); els.nearMeBtn.addEventListener('click', () => locateUser({ sortNear: true })); els.loadAllBtn.addEventListener('click', async () => { if (state.fullLoaded) { status('Full feed already loaded.'); return; } els.loadAllBtn.disabled = true; els.loadAllBtn.textContent = 'Loading all events…'; try { await loadFeed('full'); els.loadAllBtn.textContent = 'All events loaded'; } catch (error) { els.loadAllBtn.disabled = false; els.loadAllBtn.textContent = 'Load all events'; status(`Full feed failed: ${error.message}`); } }); els.majorOnly.addEventListener('change', () => { state.majorOnly = els.majorOnly.checked; savePrefs(); render(); }); els.photoOnly.addEventListener('change', () => { state.photoOnly = els.photoOnly.checked; savePrefs(); render(); }); els.nypdOnly.addEventListener('change', () => { state.nypdOnly = els.nypdOnly.checked; savePrefs(); render(); }); document.querySelectorAll('[data-cat]').forEach(input => input.addEventListener('change', () => { state.categories[input.dataset.cat] = input.checked; savePrefs(); render(); })); els.searchInput.addEventListener('input', () => { state.search = normalize(els.searchInput.value); render(); }); els.sortSelect.addEventListener('change', () => { state.sort = els.sortSelect.value; savePrefs(); if (state.sort === 'near' && !state.userLocation) locateUser({ sortNear: true }); else render(); }); document.addEventListener('click', ev => { const button = ev.target.closest('[data-copy-id]'); if (!button) return; const event = state.events.find(e => e.id === button.dataset.copyId); if (event) copyAssignment(event); }); map.on('click tap', ev => { if (ev?.originalEvent) L.DomEvent.stopPropagation(ev.originalEvent); }); }
async function boot() { loadPrefs(); syncUiFromState(); bindUi(); buildBoroughs(); buildDateChips(); if ('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js').catch(() => {}); try { await loadFeed('major'); } catch (error) { status(`Major feed failed: ${error.message}`); } }
boot();
