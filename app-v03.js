...
function googleMapsUrl(event) { return `https://www.google.com/maps/dir/?api=1&destination=${event.lat},${event.lng}&destination_place_id=&travelmode=driving`; }
...
function assignmentText(event) {
  const lines = [
    `NYCIF FIELD ASSIGNMENT`,
    event.title,
    `Time: ${timeLabel(event.start)}`,
    event.borough ? `Borough: ${event.borough}` : '',
    event.location ? `Location: ${event.location}` : '',
    distanceLabel(event) ? `Distance: ${distanceLabel(event)}` : '',
    `Photo priority: ${photoPriority(event)}`,
    crowdLabel(event) ? `Assignment read: ${crowdLabel(event)}` : '',
    isNypd(event) ? `NYPD intel item` : '',
    event.nypd_notice ? `NYPD note: ${event.nypd_notice}` : '',
    `Apple Maps: ${appleMapsUrl(event)}`,
    `Google Maps: ${googleMapsUrl(event)}`,
    (event.source_url || event.url || event.event_url) ? `Source: ${event.source_url || event.url || event.event_url}` : ''
  ];
...
async function copyAssignment(event) {
  ...
  try {
    ...
    status('Assignment copied.');
  } catch {
    window.prompt('Copy assignment:', text);
    status('Copy box opened.');
  }
}
...
function popupHtml(event) {
  ...
  return `
    <article class="popup-card">
      ...
      <div class="photo-priority ${photoPriority(event).toLowerCase().replaceAll(' ', '-')}">${esc(photoPriority(event))}</div>
      ...
      <div class="field-actions">
        <a class="field-action" target="_blank" rel="noopener" href="${esc(appleMapsUrl(event))}">Apple Maps</a>
        <a class="field-action" target="_blank" rel="noopener" href="${esc(googleMapsUrl(event))}">Google Maps</a>
        <button class="field-action" type="button" data-copy-id="${esc(event.id)}">Copy</button>
      </div>
...
function buildDateChips() {
  ...
  els.dateChips.querySelectorAll('[data-date-mode]').forEach(button => button.classList.toggle('active', button.dataset.dateMode === state.dateMode));
