(() => {
  const VERSION = 'v0.11-safe-live-test-qa';
  ...
  const state = { raw: [], enriched: [], loading: false, error: '', testedAt: null };
  ...
  function rawUrl() {
    const where = `start_date_time >= '${todayKey()}T00:00:00.000'`;
    const p = new URLSearchParams({ '$limit': '500', '$order': 'start_date_time ASC', '$where': where });
    return `${RAW_BASE}?${p.toString()}`;
  }
  ...
  async function run() {
    ...
    try {
      const [raw, enriched] = await Promise.all([getJson(rawUrl()), getJson(ENRICHED)]);
  ...
  function render() {
    ...
    body.innerHTML = `<p class="live-test-note">Read-only QA comparison. Current/future raw NYC rows only. Does not change map or saved feed.</p><dl><div><dt>Raw NYC rows sampled</dt><dd>${state.raw.length}</dd></div><div><dt>Enriched rows loaded</dt><dd>${state.enriched.length}</dd></div><div><dt>Matched raw → enriched</dt><dd>${c.matched.length} (${matchPct}%)</dd></div><div><dt>Matched with GPS</dt><dd>${c.gps.length} (${gpsPct}%)</dd></div><div><dt>Matched by event ID</dt><dd>${c.types.event_id || 0}</dd></div><div><dt>Matched by CEMSID</dt><dd>${c.types.cemsid || 0}</dd></div><div><dt>Matched by text/date</dt><dd>${c.types['text/date/location'] || 0}</dd></div><div><dt>Need enrichment</dt><dd>${c.rows.length - c.matched.length}</dd></div><div><dt>Matched but no GPS</dt><dd>${c.noGps.length}</dd></div><div><dt>Last test</dt><dd>${state.loading ? 'Testing...' : esc(tested)}</dd></div></dl>${state.error ? `<p class="live-test-error">${esc(state.error)}</p>` : ''}<button id="liveTestRunV010" type="button">Run Live Test</button><section><h3>Live rows needing enrichment</h3><ol>${c.missing.map(line).join('') || '<li>None in current sample.</li>'}</ol></section><section><h3>Matched but missing GPS</h3><ol>${c.noGps.map(line).join('') || '<li>None in current sample.</li>'}</ol></section><p class="live-test-version">${VERSION}</p>`;
