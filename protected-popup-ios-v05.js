(() => {
  ...
  function ensureStyles() {
    if (document.getElementById('nycif-popup-adsense-v05-styles')) return;
    const style = document.createElement('style');
    style.id = 'nycif-popup-adsense-v05-styles';
    style.textContent = `
  ...
  function positionPopup(popup) {
    ...
    popupEl.style.setProperty('--nycif-popup-max-height', `${Math.max(180, bottom - top - 8)}px`);
    const popupRect = popupEl.getBoundingClientRect();
    const popupWidth = Math.min(popupRect.width || 360, right - left);
    const popupHeight = Math.min(popupRect.height || 420, bottom - top);
    const desiredX = left + (right - left) / 2;
    const desiredY = top + (bottom - top) / 2;
    const centerX = clamp(desiredX, left + popupWidth / 2, right - popupWidth / 2);
    const centerY = clamp(desiredY, top + popupHeight / 2, bottom - popupHeight / 2);
    ...
    popupEl.dataset.nycifViewportMode = viewport ? 'visual-viewport-v05' : 'layout-viewport-v05';
  }
  ...
  function install() {
    ...
    if (instance.__nycifIosPopupV05) return;
    instance.__nycifIosPopupV05 = true;
    ensureStyles();
