(() => {
  const VERSION = 'nightlife-layer-v02';
  ...
  function pairFromNumbers(a, b) {
    ...
    if (isNYCoord(x, y)) return { lat: x, lng: y };
    if (isNYCoord(y, x)) return { lat: y, lng: x };
    return null;
  }
  ...
  function coordsFromObject(value) {
    ...
    if (Array.isArray(value.coordinates) && value.coordinates.length >= 2) {
      const direct = pairFromNumbers(value.coordinates[0], value.coordinates[1]);
      if (direct) return direct;
    }
    const lat = value.latitude ?? value.lat ?? value.y;
    const lng = value.longitude ?? value.lng ?? value.lon ?? value.long ?? value.x;
    const direct = pairFromNumbers(lat, lng);
    if (direct) return direct;
    for (const child of Object.values(value)) {
      if (typeof child === 'string') {
        const parsed = coordsFromText(child);
        if (parsed) return parsed;
      } else if (child && typeof child === 'object') {
        const parsed = coordsFromObject(child);
        if (parsed) return parsed;
      }
    }
    return null;
  ...
  function coordsFrom(row) {
    ...
    for (const [key, value] of Object.entries(row || {})) {
      const keyNorm = norm(key);
      if (/location|geo|geom|point|coordinate|lat|lon|lng|x|y/.test(keyNorm)) {
  ...
  function normalizeSpot(row, index) {
    ...
    const sourceId = field(row, ['serial_number', 'license_number', 'record_id', 'id', 'objectid', 'license_id', 'unique_key']) || `nightlife-${index}`;
    return {
  ...
  function buildFieldReport(rows) {
    const keys = new Map();
    let coordinateCandidates = 0;
    let categoryCandidates = 0;
    rows.slice(0, 100).forEach(row => {
      Object.keys(row || {}).forEach(key => keys.set(key, (keys.get(key) || 0) + 1));
      if (coordsFrom(row)) coordinateCandidates += 1;
      if (matchCategory(row)) categoryCandidates += 1;
    });
    return {
      sampleRows: Math.min(rows.length, 100),
      keys: [...keys.entries()].sort((a, b) => b[1] - a[1]).slice(0, 28).map(([key]) => key),
  ...
  function addFilterControl(attempt = 0) {
    ...
    debug.id = 'nightlifeDebug';
    debug.className = 'nightlife-debug';
    debug.hidden = true;
  ...
  async function fetchRows() {
    ...
    try {
      ...
      state.fieldReport = buildFieldReport(state.rows);
      const matchedRows = state.rows.filter(matchCategory).length;
      const coordRows = state.rows.filter(coordsFrom).length;
      ...
      state.stats = { rows: state.rows.length, matchedRows, coordRows, spots: spots.length, groupOne, groupTwo };
      state.loaded = true;
      if (spots.length) {
        ...
        const fields = state.fieldReport.keys.join(', ');
        status(`5PM source loaded ${state.rows.length.toLocaleString()} rows, but no mapped spots matched yet.`);
        debug(`Debug: ${matchedRows.toLocaleString()} category matches; ${coordRows.toLocaleString()} rows with coordinates. Fields seen: ${fields}`);
      }
    } catch (error) {
