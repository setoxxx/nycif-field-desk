(() => {
  const VERSION = 'v0.7.3-time-preserving-date-normalizer';
  ...
  function localDateTime(dayKey, row) {
    const parsed = timeTo24(row.start_time);
    if (parsed) return `${dayKey}T${parsed}`;
    const original = String(row.start_date_time || row.start || '');
    const timePart = original.match(/T(\d{2}:\d{2}:\d{2})/)?.[1];
    if (timePart && !/T00:00:00/.test(original)) return `${dayKey}T${timePart}`;
    return `${dayKey}T12:00:00`;
  }
  ...
  function normalizeOne(row, dayKey, suffix) {
    ...
    item.date = dayKey;
    item.start_date_time = localDateTime(dayKey, row);
  ...
  function expandRow(row) {
    ...
    const rows = [];
    for (let n = from; n <= to; n += 1) rows.push(normalizeOne(row, addDays('1970-01-01', n), n !== startN));
